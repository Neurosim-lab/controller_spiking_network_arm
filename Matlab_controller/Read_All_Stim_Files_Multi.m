...
for cell_id = 1:M_Cell_Size-1
    ...
    file_name = ['target-',num2str(target_num),'_ptype-',num2str(perturbation_type),'_pperc-',num2str(perturbation_percentage),'_cell-',num2str(cell_id-1),'_start-',num2str(stim_start_time),'_dur-',num2str(stim_duration),'_rate-',num2str(rate), '_multi'];
    file_type = '_spk.mat';
    dname=bmmFolderPerturb;
    fname=fullfile(dname,[file_name,file_type]);
    
    stim_parameters = load(fullfile(dname,[file_name '-in.txt']));
    spk = load(fname);
    %%
    ...
    [stim_number, stim_field] = size(stim_parameters);
    
    for stim_index = 1:stim_number/2
        entry_index = 2*(stim_index-1)+1;
        desired_stim(stim_parameters(entry_index,1)+1, (stim_parameters(entry_index,2) - stim_start_time +1):(stim_parameters(entry_index+1,2) - stim_start_time)) = (stim_parameters(entry_index,3)/rate)*ones(1,200);
    end
